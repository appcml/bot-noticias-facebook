    ...
    if os.path.exists(HISTORIAL_FILE):
        try:
            with open(HISTORIAL_FILE, 'r', encoding='utf-8') as f:
                ...
                print(f"📚 Historial: {len(urls)} URLs, {len(ids)} IDs, {len(titulos)} títulos")
...
def ya_publicada(noticia):
    ...
    titulo_limpio = re.sub(r'[^\w\s]', '', titulo)[:50]
...
FIN"""
    ...
    try:
        ...
        # Validar español
        palabras_es = ['el', 'la', 'de', 'que', 'y', 'en', 'un']
        if not any(p in cuerpo.lower() for p in palabras_es[:3]):
            print("   ⚠️ No generó español")
            return None, None
...
def main():
    ...
    print(f"\n🎯 {len(noticias)} noticia(s) para intentar")
