...
DEEPL_API_KEY = os.getenv('DEEPL_API_KEY')  # Para traducción profesional
...
print(f"DEBUG: DEEPL_API_KEY presente: {bool(DEEPL_API_KEY)}")
...
FUENTES_PREMIUM = {
    ...
    'politica': ['politico.com', 'axios.com', 'infobae.com', 'animalpolitico.com', 'reforma.com', 'bbc.com/news/politics']
}
...
def detectar_idioma(texto):
    ...
    palabras_esp = ['el', 'la', 'de', 'que', 'y', 'en', 'un', 'ser', 'se', 'no', 'haber', 'por', 'con', 'su', 'para', 'como', 'estar', 'tener', 'le', 'lo']
    palabras_eng = ['the', 'be', 'to', 'of', 'and', 'a', 'in', 'that', 'have', 'i', 'it', 'for', 'not', 'on', 'with', 'he', 'as', 'you', 'do', 'at']
    
    texto_lower = texto.lower()
    count_esp = sum(1 for palabra in palabras_esp if palabra in texto_lower)
    count_eng = sum(1 for palabra in palabras_eng if palabra in texto_lower)
    
    return 'EN' if count_eng > count_esp else 'ES'
...
def buscar_noticias_frescas():
    ...
    # Búsquedas enfocadas en actualidad mundial viral
    busquedas_virales = [
        ...
        ('health pandemic OR outbreak OR medical', 'salud')
    ]
    ...
    for query, categoria in busquedas_hoy:
        ...
        # Buscar en inglés (para traducir)
        ...
        try:
            ...
            # Buscar en inglés si hay API de traducción
            if DEEPL_API_KEY:
    ...
    # Eliminar duplicados por hash de URL
    noticias_unicas = {}
    ...
    noticias_nuevas = [n for n in noticias_lista if n['url'] not in HISTORIAL_URLS and n['url_hash'] not in HISTORIAL_HASHES]
    print(f"[INFO] Noticias no publicadas antes: {len(noticias_nuevas)}")
    
    # Ordenar por score (viralidad)
...
def calcular_score(art, categoria):
    ...
    # Palabras clave de alto impacto viral
    impacto_viral = {
    ...
    # Bonus por recencia (noticias de última hora)
    try:
        ...
        if fecha_pub:
            ...
            if horas_diferencia < 1:
                score += 40  # Última hora
            elif horas_diferencia < 3:
                score += 30  # Últimas 3 horas
            elif horas_diferencia < 6:
                score += 20  # Últimas 6 horas
            elif horas_diferencia < 24:
                score += 10  # Últimas 24 horas
            else:
                score -= 10  # Penalización por vieja
    except:
        pass
    
    # Bonus por imagen disponible
    if art.get('urlToImage') and art['urlToImage'].startswith('http'):
        score += 15
    
    # Bonus por categoría de crisis/emergencia
...
def subir_imagen_facebook(image_path):
    """Sube imagen a Facebook y retorna el ID"""
    ...
    try:
        ...
        with open(image_path, 'rb') as img_file:
            ...
            if 'id' in result:
                print(f"[IMAGEN] Subida exitosa: {result['id']}")
                return result['id']
            else:
                print(f"[IMAGEN] Error: {result}")
                
    except Exception as e:
        print(f"[IMAGEN] Error subiendo: {e}")
...
def generar_redaccion_inteligente(noticia, categoria):
    ...
    # Aperturas profesionales por categoría y tono
    aperturas = {
        'internacional': {
            'grave': [
    ...
    # Indicador de traducción si aplica
...
def generar_titular_prensa(noticia):
    ...
    # Plantillas de titulares por categoría
    plantillas = {
        'crisis': [
            "URGENTE: {titulo}",
            "ÚLTIMA HORA: {titulo}",
    ...
    # Limpiar el título original de posibles prefijos duplicados
...
def generar_hashtags(titulo, categoria):
    ...
    # Hashtags geográficos contextuales
...
def publicar_en_facebook():
    ...
    for noticia in noticias:
        try:
            ...
            # Preparar imagen si está disponible
            image_id = None
            image_path = None
            ...
            if noticia.get('urlToImage'):
                ...
                if image_path:
                    image_id = subir_imagen_facebook(image_path)
            ...
            # Si tenemos imagen subida, usar attached_media
            if image_id:
                payload['attached_media'] = json.dumps([{'media_fbid': image_id}])
